...
class Cropper:
    ...
    * `fix_gamma`: `bool`, default=`True`
    ...
    def __init__(
        ...
        padding=None,
        fix_gamma=True,
        facecropper_label_path='',
        facecropper_model_path='',
        device=None
        ...
        # XML Resource
        # directory = os.path.dirname('./autocrop/autocrop/')

        self.casc_path = os.path.join('./autocrop/autocrop/', CASCFILE)
        ...
        class_names = [name.strip() for name in open(facecropper_label_path).readlines()]

        net = create_Mb_Tiny_RFB_fd(len(class_names), is_test=True, device=device)
        self.predictor = create_Mb_Tiny_RFB_fd_predictor(net, candidate_size=1500, device=device)
        net.load(facecropper_model_path)
    ...
    def crop(self, path_or_array):
        ...
        -------
        if face detected:
            if face percent over 35%:
                return 1 no need face crop
            else:
        ...
        if len(faces) > 0:
            x, y, w, h = faces[-1]
        # no face detected, try another mothed
        else: 
            boxes, labels, probs = self.predictor.predict(image, 1500 / 2, 0.6)

            if len(probs) == 0:
                #really no face detected
                return 0
        ...
        org_face_percent = (w*h)/(img_height*img_width)

        if org_face_percent > 0.35: #face percent over 35%, no need to crop
            return 1
